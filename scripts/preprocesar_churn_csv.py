...
INPUT_FILE = Path("data/02_Base_WA_Fn-UseC_-Telco-Customer-Churn.csv")

# Nombre del archivo Excel que se generará después del preprocesamiento.
OUTPUT_FILE = Path("data/Telco-Customer-Churn_preprocesado.xlsx")
...
# ------------------------------------------------------------
# Cambia este valor según el tipo de codificación que quieras usar:
#
...
def cargar_dataset(ruta_archivo: Path) -> pd.DataFrame:
    """
    Lee el archivo CSV de datos de clientes y aplica las primeras limpiezas.
    ...
    3. Limpia las variables categóricas específicas.
    4. Filtra la variable objetivo 'Churn' para conservar solo valores 'SI' y 'NO'.
    5. Convierte 'Churn' a formato numérico:
       - 'SI' se transforma en 1;
    ...
    df["onlinesecurity"] = df["onlinesecurity"].astype(str).str.strip()
    df["onlinebackup"] = df["onlinebackup"].astype(str).str.strip()
    df["deviceprotection"] = df["deviceprotection"].astype(str).str.strip()
    df["techsupport"] = df["techsupport"].astype(str).str.strip()
    df["streamingtv"] = df["streamingtv"].astype(str).str.strip()
    df["streamingmovies"] = df["streamingmovies"].astype(str).str.strip()
    df["contract"] = df["contract"].astype(str).str.strip()
    df["paperlessbilling"] = df["paperlessbilling"].astype(str).str.strip()
    df["paymentmethod"] = df["paymentmethod"].astype(str).str.strip()


    # La variable objetivo se limpia y se transforma a mayúsculas
    # para trabajar de forma uniforme con los valores SI y NO.
    df["churn"] = df["churn"].astype(str).str.strip().str.upper()

    # Se conservan únicamente los registros donde 'Churn' sea SI o NO.
...
def guardar_dataset_preprocesado(X_codificado: pd.DataFrame, y: pd.Series, ruta_salida: Path) -> pd.DataFrame:
    ...
    df_preprocesado.to_excel(ruta_salida, index=False)
...
def ejecutar_preprocesamiento(metodo_codificacion: str = "onehot") -> pd.DataFrame:
    ...
    5. Guardar el archivo preprocesado como Excel.
